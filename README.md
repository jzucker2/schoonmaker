...
# Auto-label PR for daily release when head branch starts with writing/
...
Copy **`examples/requirements-ci.txt`** to the other repo root. Add **`examples/github-actions-fdx-changes-pr.yml`** to **`.github/workflows/`** for pull requests. Optionally add **`examples/github-actions-fdx-changes-push.yml`** for pushes to `main`. To auto-add the **`release-ready`** label when a PR’s head branch starts with **`writing/`**, copy **`examples/github-actions-fdx-label-pr.yml`** (runs **`schoonmaker ci-label-pr`** on `opened`/`reopened`). For a scheduled merge + patch GitHub Release with an FDX report in the notes, copy **`examples/github-actions-fdx-daily-release.yml`** only (it runs **`schoonmaker ci-daily-release`** after install; label **`release-ready`**, see **`examples/README.md`**). Edit **`requirements-ci.txt`** (**pin a tag or commit**, replace `YOUR_ORG`). For private repos, use a [`pip` URL with a token](https://pip.pypa.io/en/stable/topics/authentication/) or vendor a wheel.
